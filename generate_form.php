...
if (!has_capability('moodle/course:manageactivities', context_course::instance($courseid))) {
    echo $OUTPUT->notification(get_string('nopermission', 'local_lessonation'), 'notifyproblem');
    echo $OUTPUT->single_button(new moodle_url('/course/view.php?id=' . $courseid), get_string('back'));
    echo $OUTPUT->footer();
    exit;
}
